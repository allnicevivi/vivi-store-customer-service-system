...
| response-delivery | responses.completed | WebSocket/SSE 推播 + MongoDB 儲存 |
...
### Router Agent — 為什麼用單次結構化輸出？

一次 Gemini 呼叫同時回傳所有決策欄位：

```python
{
  "emotion_score": 0.0–1.0,   # 情緒強度
  "intent": "order_inquiry",   # 意圖分類
  "needs_clarification": false,
  "route": "agent"             # → queries.agent 或 queries.human
}
```

- 比多工具 while loop 快且省 token
- 命中 Redis 語意快取時跳過 LLM 呼叫（零 token）
- emotion_score 作為 metadata 傳遞給下游，讓 Resolution Agent 調整回覆語氣

### Resolution Agent — 為什麼用 ReAct 迭代？

客戶問題需要**多步工具組合**才能回答：

1. `search_faq` — 語意搜尋知識庫
2. `get_policy_info` — 查詢退換貨政策
3. `escalate_to_human` — 無法解決時轉人工

每一步觀察結果決定下一步（Thought → Action → Observation），不是固定 if/else 流程。

最多迭代 3 次 → 超過進 `queries.retry`（指數退避）→ 仍失敗進 `queries.dlq`。

### 為什麼其他是普通 Consumer？

職責單純、不需要動態決策：human-consumer 建工單，retry-consumer 重試邏輯，output layer 各自負責推播/審計/計費。
...
│   │   ├── response_delivery.py # WebSocket/SSE + MongoDB
...
| 技術 | 選擇 | 理由 |
|------|------|------|
| Message Queue | Kafka（KRaft 模式） | Fan-out、DLQ、持久化；生產環境可換 GCP Pub/Sub |
| Vector DB | ChromaDB | 輕量、本地運行、真實 RAG 展示；生產換 Vertex AI Matching Engine |
| LLM | Gemini 2.0 Flash | 速度快、成本低、支援結構化輸出 |
| Agent 框架 | 手刻 while loop / ReAct | 面試時能完整解釋每一行，展示機制理解深度 |
| Semantic Cache | Redis | 相同意圖查詢命中快取，跳過 LLM 呼叫 |
| 審計儲存 | MongoDB | 儲存完整 QueryMessage 供成本分析與除錯 |
